...
if 'dummy_assignments' not in st.session_state:
    ...
    {'title': 'Algebra Quiz', 'priority': 'High', 'due_date': '2024-02-10', 'time_est': 2, 'class': 'Math', 'link': '', 'done': True, 'overdue': False},
    {'title': 'Geometry Homework', 'priority': 'Medium', 'due_date': '2024-02-15', 'time_est': 3, 'class': 'Math', 'link': 'https://www.cpm.org', 'done': False, 'overdue': False},
    {'title': 'Literature Analysis', 'priority': 'High', 'due_date': '2024-02-12', 'time_est': 4, 'class': 'English', 'link': 'https://www.noredink.com', 'done': False, 'overdue': True},
    {'title': 'Grammar Exercise', 'priority': 'Low', 'due_date': '2024-02-20', 'time_est': 1, 'class': 'English', 'link': '', 'done': False, 'overdue': False},
    {'title': 'Ancient Civilizations Research', 'priority': 'Medium', 'due_date': '2024-02-18', 'time_est': 5, 'class': 'History', 'link': 'https://www.history.com', 'done': True, 'overdue': True},
    {'title': 'World War II Essay', 'priority': 'High', 'due_date': '2024-02-14', 'time_est': 6, 'class': 'History', 'link': 'https://www.docs.google.com', 'done': True, 'overdue': False},
    ]
...
with sidebar_tabs[1]:
    ...
    with st.form(key='create_assignment'):
        st.text_input("Enter Title", max_chars=100, help='What is the assignment called?', value='Unit Circle Practice')
        st.selectbox('Choose Priority:', ['High', 'Medium', 'Low'], help='How important is this assignment?')
        st.date_input('Enter Due Date:', help='When is this assignment due?')
        st.number_input('Enter Time Estimate:', step=5, help='How long do you think this assignment will take? (in minutes)', min_value=5, max_value=600, value=30)
        st.selectbox('Enter Class:', ['Math', 'English', 'History'], help='Which class is this assignment for?')
        st.text_input('Link to assignment (Optional)', help='Enter a link to the assignment.', value=('https://www.google.com'))
...
with sidebar_tabs[2]:
    st.info(':arrow_up: This tab is used to delete classes.')
    to_be_deleted = st.selectbox('Delete Class', {assignment['class'] for assignment in st.session_state.dummy_assignments}, help='Delete a class.')
    if st.button('Delete'):
    ...
    if class_filter == 'All':
        if editing:
            assignment_df = st.data_editor(
                assignment_df,
                column_order=(['title', 'priority', 'due_date', 'link', 'time_est', 'class', 'done', 'overdue']),
                column_config={
                    ...
                    'class': st.column_config.SelectboxColumn(
                        'Class',
                        options={assignment['class'] for assignment in st.session_state.dummy_assignments},
            ...
            st.info('This :arrow_up: is a data editor that allows you to edit your assignments. Mark them as done if you would like to remove them. Edits will not be saved until you toggle out of edit mode. Overdue status is updated automatically(will not funtion in the tutorial).')
            ...
            st.info('This :arrow_up: is a table of assignments. You can edit the assignments by turning on the edit mode toggle over there :arrow_right:. The table can be searched, downloaded, or put into fullscreen mode by hovering over it and clicking on the pop-up menu')
...
with columns[1]:
    ...
    st.info("This button :arrow_up: is used to import your assignments from your browser's cookies. Note: this button will not function in this tutorial.")
    st.button('Save Assignments')
    st.info("This button :arrow_up: is used to save your assignments to your browser's cookies. Note: this button will not function in this tutorial.")